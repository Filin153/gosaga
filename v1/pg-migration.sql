...
BEGIN
    IF EXISTS (
        ...
        WHERE table_schema = 'public' AND table_name = 'go_saga_in_task' AND column_name = 'data' AND data_type <> 'bytea'
    ) THEN
        ALTER TABLE "go_saga_in_task" 
        ALTER COLUMN "data" TYPE BYTEA USING CASE WHEN "data" IS NULL THEN NULL ELSE convert_to("data"::text, 'UTF8') END;
    END IF;
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_schema = 'public' AND table_name = 'go_saga_in_task' AND column_name = 'rollback_data' AND data_type <> 'bytea'
    ) THEN
        ALTER TABLE "go_saga_in_task" 
        ALTER COLUMN "rollback_data" TYPE BYTEA USING CASE WHEN "rollback_data" IS NULL THEN NULL ELSE convert_to("rollback_data"::text, 'UTF8') END;
    END IF;
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_schema = 'public' AND table_name = 'go_saga_out_task' AND column_name = 'data' AND data_type <> 'bytea'
    ) THEN
        ALTER TABLE "go_saga_out_task" 
        ALTER COLUMN "data" TYPE BYTEA USING CASE WHEN "data" IS NULL THEN NULL ELSE convert_to("data"::text, 'UTF8') END;
    END IF;
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_schema = 'public' AND table_name = 'go_saga_out_task' AND column_name = 'rollback_data' AND data_type <> 'bytea'
    ) THEN
        ALTER TABLE "go_saga_out_task" 
        ALTER COLUMN "rollback_data" TYPE BYTEA USING CASE WHEN "rollback_data" IS NULL THEN NULL ELSE convert_to("rollback_data"::text, 'UTF8') END;
    END IF;
